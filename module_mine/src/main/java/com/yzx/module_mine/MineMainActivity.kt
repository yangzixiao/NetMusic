 ...
 */
class MineMainActivity:BaseActivity<CommonVIewModel>() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setTransparentStatus()
        setStatusDarkFont()
        setContentView(ActivityMineMainBinding.inflate(layoutInflater).root)
        supportFragmentManager.beginTransaction().replace(R.id.fragmentContainer,ARouterNavUtils.getFragment(FRAGMENT_MINE)).commit()
    }
}