package com.yzx.lib_multitype_adapter
...
import com.yzx.lib_multitype_adapter.callback.DiffItemCallback
import com.yzx.lib_multitype_adapter.holder.MultiTypeViewHolder
...
class MultiTypeAdapter constructor(val layoutManager: RecyclerView.LayoutManager) :
    ...
    private val mAsyncListChange by lazy {
        AsyncListDiffer(this, DiffItemCallback<MultiTypeBinder<*>>())
    }
    ...
    fun notifyItemDataChanged(binder: MultiTypeBinder<*>) {

        mHashCodeViewType.forEach {
            if (it.key==binder.hashCode()) {
                mHashCodeViewType[it.key]=binder
            }
        }
        mAsyncListChange.submitList(mHashCodeViewType.map { it.value })
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MultiTypeViewHolder {
        try {
            return MultiTypeViewHolder(
                parent.inflateDataBinding(mHashCodeViewType[viewType]?.layoutId()!!))
        } catch (e: Exception) {
